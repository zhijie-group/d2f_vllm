...
from d2f_engine.engine.sequence import SequenceForCausalLM, SequenceForDiffusionLM, SequenceBase
from d2f_engine.models.auto_model import AutoModelLM
from d2f_engine.layers.sampler import AutoSampler
from d2f_engine.utils.checker import CHECK_SLOT_MAPPING
...
class ModelRunnerForDiffusionLM(ModelRunnerBase):
    ...
    def allocate_kv_cache(self):
        ...
        peak = torch.cuda.memory_stats()["allocated_bytes.all.peak"]
        current = torch.cuda.memory_stats()["allocated_bytes.all.current"]
        num_kv_heads = getattr(hf_config, "num_key_value_heads", getattr(hf_config, "n_kv_heads", None)) // self.world_size
