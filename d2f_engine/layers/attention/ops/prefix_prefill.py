...
# The kernels in this file are originally adapted from LightLLM's context_attention_fwd:
...
@triton.jit
def _fwd_kernel_d2f(Q, K, V, Mask,
                    K_cache, V_cache,
                    B_Loc,
    ...
    for start_n in tl.range(0, cur_batch_ctx_len, BLOCK_SIZE, loop_unroll_factor=num_unroll_cache):
        ...
        # [BLOCK_SIZE,D]
        offs_v = (bn[:, None] * stride_v_cache_bs + cur_kv_head * stride_v_cache_h + 
                  offs_d[None, :] * stride_v_cache_d + offs_bs_n[:, None] * stride_v_cache_bl)

        if start_n + BLOCK_SIZE > cur_batch_ctx_len or BLOCK_DMODEL != BLOCK_DMODEL_PADDED:
            k_load = tl.load(K_cache + offs_k, 
...
def context_attention_fwd(q,
    ...
    if diffusion_blk_sz is None:
        ...
        _fwd_kernel_d2f[GRID](
            ...
            *v.stride(),
            *o.stride(),
            *k_cache.stride(), #[num_blocks, num_kv_heads, head_size/x, block_size, x]
            *v_cache.stride(), #[num_blocks, num_kv_heads, head_size, block_size]
            *mask.stride(),
